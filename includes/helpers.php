...
if (!function_exists('acowdp_register_rest_route')) {
     ...
     * @param callable|string $permission Permission callback or function name.
     */
    function acowdp_register_rest_route($route, $handler, $method = 'GET', $permission = '__return_true') {
        $method_map = [
        ...
        register_rest_route(
            defined('ACOWDP_REST_NAMESPACE') ? ACOWDP_REST_NAMESPACE : 'acowdp-api/v1',
            $route,
            [
                'methods'             => $method_map[$method] ?? $method,
                'callback'            => $callback,
                'permission_callback' => is_callable($permission) ? $permission : function() { return true; },
            ]
        );
    }
}